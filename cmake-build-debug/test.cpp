                            ...
                            size_t k, const string & message)
{
    if(seq == seq_truth) {
        ...
        cout << "Passed " << message <<
             " (assembled a sequence of the same composition with the original
        sequence). Congratulations!" << endl;
    } else {
...
// The value of k for k-mers to be used for each test sequence:
    ...
    for (size_t i=0; i<seqs_truth.size(); ++i) {
        ...
        if(method == "k-mer pairwise comparison") {
            ...
            create_deBruijn_graph_by_h1ashing(kmers, g);
        } else {
            cerr << "ERROR: unknown methods!" << endl;
        }
        clock_t end = clock();
        double elapsed_secs = double(end - begin) / CLOCKS_PER_SEC;
        cout << "Elapsed time for building de Bruijn graph: " << elapsed_secs <<
             endl;
        if(has_Eulerian_path(g)) {
            std::cout << "Passed test for existence of Eulerian path.
